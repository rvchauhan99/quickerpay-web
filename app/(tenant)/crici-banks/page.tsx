...
import type { MerchantListItem, Pagination, CriciBankListItem } from '@quickerpay/shared-types'
...
export default function CriciBanksPage() {
  const { ready, user, menus, accessToken, allowed, Forbidden } = useTenantScreen('CRICI_BANKS')
  ...
  const [rows, setRows] = useState<CriciBankListItem[]>([])
  ...
  const canEdit = hasMenu(menus, 'CRICI_BANKS', 'can_edit')
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await apiListRequest<CriciBankListItem>(`/api/v1/crici/banks?${query}`, { token: accessToken })
  ...
  const handleStatus = async (id: string, active: boolean) => {
    ...
    setSubmitting(id)
    try {
      await apiRequest(`/api/v1/crici/banks/${id}/status`, {
        method: 'POST',
        token: accessToken,
        body: { active },
      })
  ...
  const handleOwnerChange = async (id: string, ownerUserId: string) => {
    ...
    try {
      await apiRequest(`/api/v1/crici/banks/${id}`, {
        method: 'PATCH',
        token: accessToken,
  ...
  return (
    <AppShell title="Crici Banks" role={user.role} menus={menus}>
      <PageHeader
        title="Crici Banks"
        action={
          canEdit ? (
      ...
      {loading ? (
        ...
        <DataTable
          ...
          rows={rows.map((row) => ({
            ...
            method: String(row.crici_billing_method_id),
            upi: row.upi_address,
            name: row.display_name,
            type: `min ${row.min_rupees} – max ${row.max_rupees}`,
